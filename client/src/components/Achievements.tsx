...
export default function Achievements({ onChange, embedded = false }: { onChange?: (data: AchievementSnapshot) => void; embedded?: boolean }) {
  ...
  return (
    <section className={`card${embedded ? " profile-collection" : ""}`} aria-labelledby="achievements-heading">
      ...
      <p className="muted small">Campaign and profile achievements recognize your existing saved data. Spectator memberships and NPCs do not count; Behind the Screen requires the DM role. Character milestones count player characters you own in campaigns you still belong to. Dressed to Impress requires an owned, non-default effect equipped from the shop. Existing quest credit counts toward Quest Regular; your saved minimum streak continues toward three. Once earned, badges remain unlocked.</p>
      </details>
